...
def OHLC_S_F_2(dataframe, col_open, col_high, col_low, col_close):
    ...
    def plot_ohlc(change=None):
        ...
        if forecast_checkbox.value:
            ...
            for _ in range(len(predict_model)):
                if (_ > 0) and (Y_test.iloc[_] > Y_test.iloc[_-1]) and (predict_model[_] >= Y_test.iloc[_]):
                    ...
                    ax[0].scatter(index[_], predict_model[_], s=abs(Y_test.iloc[_] - predict_model[_]), linewidth=1.5, label='False Forecast', edgecolor=pal_black[5], color=pal_black[1])
        ...
        numbers = new_dataframe.iloc[-data_pred:,0]
        average = sum(numbers)/len(numbers)
        ax[1].axhline(average, linestyle='--', linewidth=0.7, color=pal_black[0])
